...
AUTH_USERS = {
    ...
    "tanya": {
        "hash": "8cc6f49a2d9377bbc112f411819deafb19790553c170cb7af4b6d7c53e296b80",
        "role": "user",
        "name": "Таня",
        "token": "2ed06c9ca37d0493",
    },
    "demo": {
...
def show_login_page():
    ...
    st.markdown("""<style>
        ...
        /* All form buttons — override Streamlit primary green */
        [data-testid="stFormSubmitButton"] > button,
        [data-testid="stFormSubmitButton"] > button[kind="primary"],
        [data-testid="stFormSubmitButton"] > button[kind="secondaryFormSubmit"],
        .stFormSubmitButton > button {
            background: rgba(255,255,255,0.06) !important; color: #7a7a92 !important;
        ...
        .stFormSubmitButton > button:hover {
            ...
            border-color: rgba(255,255,255,0.15) !important; box-shadow: none !important;
        }
        /* "Войти" button — first/wider column: subtle accent */
        [data-testid="stColumn"]:first-child [data-testid="stFormSubmitButton"] > button,
        [data-testid="stColumn"]:first-child [data-testid="stFormSubmitButton"] > button[kind="primary"] {
            background: linear-gradient(135deg, rgba(0,255,106,0.10), rgba(0,255,106,0.18)) !important;
            color: #00ff6a !important; font-size: 1rem !important;
            border: 1px solid rgba(0,255,106,0.2) !important; box-shadow: none !important;
        }
        [data-testid="stColumn"]:first-child [data-testid="stFormSubmitButton"] > button:hover {
            background: linear-gradient(135deg, rgba(0,255,106,0.18), rgba(0,255,106,0.28)) !important;
            color: #fff !important; border-color: rgba(0,255,106,0.35) !important;
            box-shadow: 0 4px 15px rgba(0,255,106,0.08) !important;
        }
    ...
    with col2:
        ...
        <div style="text-align:center; margin-bottom: 10px;">
            ...
            <p style="color: #555568; font-size: 0.65rem; text-transform:uppercase; letter-spacing:.15em;">Интеллект ресторана</p>
        </div>
...
/* Desktop: minimal header — hide menus but keep sidebar controls working */
...
/* Sidebar collapse button must be fully clickable */
...
/* Period selector: cap width on desktop */
...
@media (max-width: 768px) {
    ...
    /* --- Hide Streamlit header completely on mobile, remove top gap --- */
    header[data-testid="stHeader"] {
        display: none !important; height: 0 !important;
    }
    /* Sidebar collapse button must work on mobile */
    [data-testid="stSidebarCollapseButton"] {
    ...
    .block-container {
        padding: 8px 10px 16px 10px !important; max-width: 100% !important;
    }
    [data-testid="stMainBlockContainer"] {
        padding-top: 50px !important;
    }
    ...
    /* Period selector — exclude from 2-col grid, handled by JS MutationObserver */
    /* Allow 2-column for small pairs (metrics row) */
    div[data-testid="stHorizontalBlock"] > div[data-testid="stColumn"] {
        flex: 1 1 calc(50% - 4px) !important; min-width: calc(50% - 4px) !important;
    }
    /* Period selector: override 2-col grid — allow natural flex sizing */
    div[data-testid="stHorizontalBlock"]:has([data-testid="stSelectbox"]) > div[data-testid="stColumn"] {
        flex: 0 0 auto !important; min-width: 36px !important;
    }
    div[data-testid="stHorizontalBlock"]:has([data-testid="stSelectbox"]) > div[data-testid="stColumn"]:has([data-testid="stSelectbox"]) {
        flex: 1 1 auto !important; max-width: 160px !important; min-width: 90px !important;
    }
    div[data-testid="stHorizontalBlock"]:has([data-testid="stSelectbox"]) > div[data-testid="stColumn"]:has([data-testid="stDateInput"]) {
        flex: 0 1 90px !important; max-width: 100px !important; min-width: 70px !important;
    }
    div[data-testid="stHorizontalBlock"]:has([data-testid="stSelectbox"]) {
...
if IS_LIGHT:
    ...
    /* Force icon button text color in light theme */
    section[data-testid="stSidebar"] .stButton > button > div,
    section[data-testid="stSidebar"] .stButton > button > div > p,
    section[data-testid="stSidebar"] .stButton > button > div > div,
    section[data-testid="stSidebar"] .stButton > button span,
...
(function() {
    ...
    // Fix period selector columns on mobile
    function fixPeriodSelector() {
        pdoc.querySelectorAll('[data-testid="stHorizontalBlock"]').forEach(block => {
            const selectbox = block.querySelector('[data-testid="stSelectbox"]');
            if (!selectbox) return;
            ...
            if (isMobile()) {
                ...
                cols.forEach(col => {
                    const hasSelect = col.querySelector('[data-testid="stSelectbox"]');
                    const hasDate = col.querySelector('[data-testid="stDateInput"]');
                    const hasBtn = col.querySelector('button');
                    ...
                    if (hasSelect) {
                        ...
                        col.style.setProperty('min-width', '90px', 'important');
                    } else if (hasDate) {
                        col.style.setProperty('flex', '0 1 90px', 'important');
                        col.style.setProperty('max-width', '100px', 'important');
                        col.style.setProperty('min-width', '70px', 'important');
                    } else if (hasBtn) {
                        col.style.setProperty('flex', '0 0 40px', 'important');
                        col.style.setProperty('min-width', '40px', 'important');
                        col.style.setProperty('max-width', '44px', 'important');
                    } else {
                        col.style.setProperty('display', 'none', 'important');
                    }
                });
            } else {
                block.style.setProperty('max-width', '320px', 'important');
            }
        });
    }
    fixPeriodSelector();
    ...
    // === CUSTOM HAMBURGER (mobile only) ===
    function createHamburger() {
        if (!isMobile()) return;
        let h = pdoc.getElementById('_mob_hamburger');
        if (h) { h.style.display = 'flex'; return; }
        h = pdoc.createElement('div');
        h.id = '_mob_hamburger';
        h.innerHTML = '&#9776;';
        h.style.cssText = 'position:fixed;top:10px;left:10px;z-index:999999;width:40px;height:40px;'+
            'border-radius:10px;display:flex;align-items:center;justify-content:center;'+
            'font-size:20px;cursor:pointer;user-select:none;-webkit-tap-highlight-color:transparent;'+
            'background:rgba(20,20,31,0.85);color:#00ff6a;border:1px solid rgba(255,255,255,0.08);'+
    ...
    function updateHamburgerTheme() {
        const h = pdoc.getElementById('_mob_hamburger');
        if (!h) return;
        const isLight = pdoc.body.classList.contains('light') ||
            pwin.getComputedStyle(pdoc.documentElement).getPropertyValue('--bg').trim().startsWith('#f');
        if (isLight) {
            h.style.background = 'rgba(255,255,255,0.9)';
    ...
    function clearMainContent() {
        ...
        if (main) {
            ...
            const mainSection = pdoc.querySelector('[data-testid="stMain"]');
            if (mainSection) mainSection.scrollTop = 0;
            let ov = pdoc.getElementById('_nav_overlay');
            if (!ov) {
    ...
    if (pdoc.readyState === 'complete') attachNavListeners();
    else pwin.addEventListener('load', attachNavListeners);
    new MutationObserver(attachNavListeners).observe(pdoc.body, {childList:true, subtree:true});
})();
</script>""", height=0)
...
with st.sidebar:
    ...
    # --- Action buttons row (compact, Unicode symbols) — 2 rows ×2 + 1 for even grid ---
    ...
    with _bc2:
        ...
        if st.button(_ti, key="theme_btn", help=t("btn_theme"), use_container_width=True):
    ...
    with _bc3:
        ...
        if st.button(_lang_icon, key="lang_btn", help="English / Русский", use_container_width=True):
            st.session_state["_lang"] = "en" if _get_lang() == "ru" else "ru"
            st.rerun()
    with _bc4:
        if st.button("⚙", key="nav_lk_top", help=t("btn_account"), use_container_width=True):
            st.session_state["_page"] = "Личный кабинет"
            st.rerun()
    _bc5, _bc6 = st.columns(2)
    with _bc5:
        if st.button("↗", key="logout_btn", help=t("btn_logout"), use_container_width=True):
            st.session_state.pop("_auth_user", None)
            st.rerun()

    # --- JS: force ALL button/tooltip/table colors for current theme ---
    _sb_c = "#5a5a70" if IS_LIGHT else "#bbbbc8"
    _sb_active_c = "#00b847" if IS_LIGHT else "#00ff6a"
    _sb_active_bg = "rgba(0,180,80,0.08)" if IS_LIGHT else "rgba(0,255,106,0.06)"
    _sb_hover_bg = "rgba(90,90,112,0.08)" if IS_LIGHT else "rgba(187,187,200,0.08)"
    _main_c = "#1a1a2e" if IS_LIGHT else "#bbbbc8"
    _main_bg = "transparent" if IS_LIGHT else "transparent"
    _main_border = "1px solid rgba(90,90,112,0.25)" if IS_LIGHT else "1px solid rgba(187,187,200,0.2)"
    _main_hover_bg = "rgba(90,90,112,0.08)" if IS_LIGHT else "rgba(187,187,200,0.08)"
    _main_disabled_c = "rgba(90,90,112,0.4)" if IS_LIGHT else "rgba(187,187,200,0.4)"
    # tooltip theme colors
    _tip_bg = "#eeeef2" if IS_LIGHT else "#1e1e2e"
    _tip_c = "#1a1a2e" if IS_LIGHT else "#e0e0e8"
    # table / dataframe theme colors
    _tbl_bg = "#ffffff" if IS_LIGHT else "#0e0e16"
    _tbl_c = "#1a1a2e" if IS_LIGHT else "#ffffff"
    _tbl_c2 = "#5a5a70" if IS_LIGHT else "#7a7a92"
    _tbl_hdr_bg = "rgba(0,180,80,0.04)" if IS_LIGHT else "rgba(0,255,106,0.03)"
    _tbl_border = "rgba(0,0,0,0.08)" if IS_LIGHT else "rgba(255,255,255,0.04)"
    _gdg_cell = "#ffffff" if IS_LIGHT else "#0e0e16"
    _gdg_header = "#f8f8fa" if IS_LIGHT else "#14141f"
    _gdg_text = "#1a1a2e" if IS_LIGHT else "#ffffff"
    _gdg_text_med = "#5a5a70" if IS_LIGHT else "#7a7a92"
    _gdg_border = "rgba(0,0,0,0.08)" if IS_LIGHT else "rgba(255,255,255,0.04)"
    _components.html(f"""<script>
    (function(){{
        var d=window.parent.document;
        var styleId='goai-theme-fix';
        function injectCSS(){{
            var old=d.getElementById(styleId);
            if(old) old.remove();
            var s=d.createElement('style');
            s.id=styleId;
            s.textContent=`
                /* === SIDEBAR BUTTONS === */
                section[data-testid="stSidebar"] .stButton button:not([kind="headerNoPadding"]) {{
                    background: transparent !important;
                    background-color: transparent !important;
                    border: none !important;
                    box-shadow: none !important;
                    color: {_sb_c} !important;
                }}
                section[data-testid="stSidebar"] .stButton button:not([kind="headerNoPadding"]):hover {{
                    background: {_sb_hover_bg} !important;
                    background-color: {_sb_hover_bg} !important;
                    color: {_sb_active_c} !important;
                    border: none !important;
                }}
                section[data-testid="stSidebar"] .stButton button:not([kind="headerNoPadding"]):active {{
                    background: {_sb_active_bg} !important;
                    background-color: {_sb_active_bg} !important;
                    color: {_sb_active_c} !important;
                }}
                section[data-testid="stSidebar"] .stButton button[kind="primary"] {{
                    background: {_sb_active_bg} !important;
                ...
                [data-testid="stMain"] .stButton button:not([kind="headerNoPadding"]) {{
                    background: {_main_bg} !important;
                    background-color: {_main_bg} !important;
                    color: {_main_c} !important;
                    border: {_main_border} !important;
                    box-shadow: none !important;
                }}
                [data-testid="stMain"] .stButton button:not([kind="headerNoPadding"]):hover {{
                    background: {_main_hover_bg} !important;
                    background-color: {_main_hover_bg} !important;
                    color: {_main_c} !important;
                    border: {_main_border} !important;
                }}
                [data-testid="stMain"] .stButton button:not([kind="headerNoPadding"]):active {{
                    background: rgba(0,180,80,0.12) !important;
                    background-color: rgba(0,180,80,0.12) !important;
                }}
                [data-testid="stMain"] .stButton button:disabled,
                [data-testid="stMain"] .stButton button:disabled:hover {{
                    background: transparent !important;
                    background-color: transparent !important;
                    color: {_main_disabled_c} !important;
                    border: 1px solid transparent !important;
                }}
                /* === TOOLTIPS === */
                [class*="stTooltipContent"] {{
                    color: {_tip_c} !important;
                }}
                [class*="stTooltipContent"] * {{
                    color: {_tip_c} !important;
                }}
                div:has(> [class*="stTooltipContent"]) {{
                    background-color: {_tip_bg} !important;
                    background: {_tip_bg} !important;
                    color: {_tip_c} !important;
                }}
                .stTooltipIcon div[class*="st-"] {{
                    background-color: {_tip_bg} !important;
                    background: {_tip_bg} !important;
                    color: {_tip_c} !important;
                }}
                /* === TABLES (st.table HTML) === */
                table {{
                    background: {_tbl_bg} !important;
                    background-color: {_tbl_bg} !important;
                    border-color: {_tbl_border} !important;
                }}
                table th {{
                    background: {_tbl_hdr_bg} !important;
                    background-color: {_tbl_hdr_bg} !important;
                    color: {_tbl_c} !important;
                    border-color: {_tbl_border} !important;
                }}
                table td {{
                    color: {_tbl_c2} !important;
                    border-color: {_tbl_border} !important;
                    background: transparent !important;
                    background-color: transparent !important;
                }}
                ...
                [data-testid="stDataFrame"] [data-testid="glideDataEditor"] {{
                    --gdg-bg-cell: {_gdg_cell} !important;
                    --gdg-bg-header: {_gdg_header} !important;
                    --gdg-bg-header-has: {_gdg_header} !important;
                    --gdg-text-dark: {_gdg_text} !important;
                    --gdg-text-medium: {_gdg_text_med} !important;
                    --gdg-border-color: {_gdg_border} !important;
                    --gdg-bg-cell-medium: {_gdg_cell} !important;
                }}
            `;
        ...
        function fixInline(){{
            ...
            if(sb){{
                sb.querySelectorAll('.stButton button').forEach(function(btn){{
                    if(btn.getAttribute('kind')==='headerNoPadding') return;
                    var isPrimary=btn.getAttribute('kind')==='primary';
                    var c=isPrimary?'{_sb_active_c}':'{_sb_c}';
                    btn.querySelectorAll('div,p,span').forEach(function(el){{
                        el.style.setProperty('color',c,'important');
                    }});
                }});
            }}
            var mn=d.querySelector('[data-testid="stMain"]');
            if(mn){{
                mn.querySelectorAll('.stButton button').forEach(function(btn){{
                    if(btn.getAttribute('kind')==='headerNoPadding') return;
                    btn.querySelectorAll('div,p,span').forEach(function(el){{
                        el.style.setProperty('color','{_main_c}','important');
                    }});
                }});
            }}
            /* Fix tooltip bubbles — search broadly */
            d.querySelectorAll('[class*="stTooltipContent"]').forEach(function(tc){{
                tc.style.setProperty('color','{_tip_c}','important');
                var bubble=tc.parentElement;
                if(bubble){{
                    bubble.style.setProperty('background-color','{_tip_bg}','important');
                    bubble.style.setProperty('background','{_tip_bg}','important');
                    bubble.style.setProperty('color','{_tip_c}','important');
                }}
            }});
            /* Fix all divs inside tooltip icons that may have stale bg */
            d.querySelectorAll('.stTooltipIcon div').forEach(function(el){{
                var cs=window.getComputedStyle(el);
                var bg=cs.backgroundColor;
                if(bg && bg!=='rgba(0, 0, 0, 0)' && bg!=='transparent'){{
                    el.style.setProperty('background-color','{_tip_bg}','important');
                    el.style.setProperty('background','{_tip_bg}','important');
                    el.style.setProperty('color','{_tip_c}','important');
                }}
            }});
        }}
        injectCSS();
        fixInline();
        setTimeout(fixInline,100);
        setTimeout(fixInline,300);
        setTimeout(fixInline,800);
        setTimeout(fixInline,2000);
        new MutationObserver(function(){{ fixInline(); }}).observe(d.body,{{childList:true,subtree:true}});
    }})();
    </script>""", height=0)
...
if page == "Выручка":
    ...
    if orders.empty:
        ...
        # --- Индикатор загрузки для секций ниже ---
        _loading_ph = st.empty()
        _loading_ph.markdown("""<div style="text-align:center;padding:32px 0;">
            <div style="display:inline-block;width:24px;height:24px;border:3px solid var(--border,rgba(255,255,255,0.1));
                border-top-color:var(--green,#00ff6a);border-radius:50%;animation:_spin 0.8s linear infinite;"></div>
            <div style="color:var(--t3,#888);font-size:.78rem;margin-top:8px;">Загрузка…</div>
...
- Рубли в формате 1 234 ₽. Русский язык. Без воды — только суть."""
        ...
        # Remove loading indicator — page fully loaded
        _loading_ph.empty()
