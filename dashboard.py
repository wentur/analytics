...
(function() {
    ...
    function clearMainContent() {
        ...
        if (main) {
            ...
            if (!ov) {
                ...
                ov.id = '_nav_overlay';
                ov.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:var(--bg,#fff);';
    ...
    new MutationObserver(function() {
        ...
        _navTimer = setTimeout(restoreMainContent, 150);
...
with st.sidebar:
    ...
    st.markdown(f'<div style="font-size:.7rem;color:{_cap_color};margin:-8px 0 0;">Интеллект ресторана</div>', unsafe_allow_html=True)
...
def period_selector(key_suffix="main"):
    ...
    with st.container():
        pc = st.columns([2, 1, 1, 1, 4])
        with pc[0]:
            period = st.selectbox("Период", ["Сегодня","Вчера","7 дней","30 дней","90 дней","Произвольный"],
                index=["Сегодня","Вчера","7 дней","30 дней","90 дней","Произвольный"].index(st.session_state["_period"]),
                key=f"_period_sel_{key_suffix}", label_visibility="collapsed")
            st.session_state["_period"] = period
        if period == "Произвольный":
            with pc[1]:
                d1 = st.date_input("С", st.session_state["_date_from"], key=f"_d1_{key_suffix}", label_visibility="collapsed")
            with pc[2]:
                d2 = st.date_input("По", st.session_state["_date_to"], key=f"_d2_{key_suffix}", label_visibility="collapsed")
        else:
        ...
        _ref_col = pc[3] if period == "Произвольный" else pc[1]
        with _ref_col:
            if st.button("↻", key=f"refresh_{key_suffix}", help="Обновить данные"):
                st.cache_data.clear()
                for k in list(st.session_state.keys()):
                    if k.startswith("_pdata_") or k.startswith("sh_purchases_") or k.startswith("_qc_") or k.startswith("_stock_"):
                        del st.session_state[k]
    ...
    for group_name, group_pages in MENU_GROUPS:
        ...
        if group_name == "":
            ...
            st.markdown(f'<div style="font-size:.62rem;font-weight:700;text-transform:uppercase;'
                        f'letter-spacing:.12em;color:var(--t3);padding:12px 10px 4px;">{group_name}</div>',
                        unsafe_allow_html=True)
            for p in visible:
                active = st.session_state["_page"] == p
                if st.button(p, key=f"nav_{p}", use_container_width=True,
...
if page == "Пульс":
    ...
    with c2:
        ...
        st.metric("Заказов", f"{orders_period:,}", delta_fmt(d))
    with c3:
    ...
    if not _has_cost_data:
        st.warning("Данные о закупках не загружены — маржа и доход завышены.")
        if st.button("📥 Загрузить за 30 дней", key="load_purchases_btn"):
    ...
    st.caption("Маржа = Выручка − Себестоимость. Доход = Маржа − НДС − Постоянные расходы.")
    ...
    with left:
        st.markdown("### Выручка: сегодня vs вчера")
    ...
    if not rest_data.empty and len(rest_data) > 5:
        ...
        fig = px.bar(rest_data, x="REVENUE", y="REST_NAME", orientation="h",
            ...
            labels={"REST_NAME": "Точка", "REVENUE": "Выручка, ₽"})
    ...
    if orders.empty:
        ...
        def _vyr_delta(cur, prev):
            if prev and prev > 0:
                ...
                _lbl = "vs вчера" if _period_days == 1 else "vs пред."
                return f"{pct:+.1f}% {_lbl}"
            return None
        c1,c2,c3,c4,c5,c6 = st.columns(6)
        with c1: st.metric("Выручка",f"{rev:,.0f} ₽", delta=_vyr_delta(rev, _y_rev))
        with c2: st.metric("Заказов",f"{n:,}", delta=_vyr_delta(n, _y_n))
        with c3: st.metric("Чеков",f"{n_checks:,}", delta=_vyr_delta(n_checks, _y_checks))
        with c4: st.metric("Ср. чек",f"{avg_c:,.0f} ₽", delta=_vyr_delta(avg_c, _y_avg))
        with c5: st.metric("Гостей",f"{guests:,}", delta=_vyr_delta(guests, _y_guests))
        with c6: st.metric("Блюд",f"{dishes_n:,}", delta=_vyr_delta(dishes_n, _y_dishes))
        ...
        if not tax_data.empty and "REVENUE" in tax_data.columns:
            ...
            tax_data["TAX_LABEL"] = tax_data["TAX_NAME"].fillna("Не указан")
            ...
            with tc1:
                fig_tax = px.bar(tax_data, x="TAX_LABEL", y="REVENUE",
                    ...
                    labels={"TAX_LABEL": "Ставка НДС", "REVENUE": "Выручка, ₽", "SHARE": "Доля %"})
            ...
            with tc2:
                ...
                for _, row in tax_data.iterrows():
                    ...
                    _tax_line = f'<div style="font-size:.78rem;font-weight:600;color:{"#f59e0b" if not IS_LIGHT else "#b45309"};margin-top:4px;">НДС: {_tax_amt:,.0f} ₽</div>' if _rate > 0 else ""
                    st.markdown(f'<div style="margin-bottom:12px;">'
                        f'<div style="font-size:.75rem;color:var(--t3);text-transform:uppercase;letter-spacing:.08em;font-weight:600;">{label}</div>'
                        ...
                        f'</div>'
                        f'<div style="font-size:.7rem;color:var(--t3);margin-top:3px;">{qty:,} позиций</div>'
                        f'</div>', unsafe_allow_html=True)

            # --- Детализация налогов: по ресторанам и товарам ---
            with st.expander("Детализация по ставкам НДС"):
                _tax_tab1, _tax_tab2 = st.tabs(["По ресторанам", "По товарам"])
                with _tax_tab1:
                    ...
                    if not _tax_by_rest.empty:
                        ...
                        for _tl in _tax_by_rest["TAX_LABEL"].unique():
                            ...
                            st.markdown(f"**{_tl}** — выручка {_total_rev:,.0f} ₽, НДС {_tax_total:,.0f} ₽")
                            ...
                            _grouped.columns = ["Ресторан", "Выручка ₽", "Позиций"]
                            _grouped = _grouped.sort_values("Выручка ₽", ascending=False)
                            _grouped["Выручка ₽"] = _grouped["Выручка ₽"].apply(lambda x: f"{x:,.0f}")
                            _grouped["Позиций"] = _grouped["Позиций"].astype(int)
                ...
                with _tax_tab2:
                    ...
                    if not _tax_by_dish.empty:
                        _tax_filter = st.selectbox("Ставка НДС", ["Все"] + list(_tax_by_dish["TAX_LABEL"].unique()), key="tax_dish_filter")
                        _filtered = _tax_by_dish if _tax_filter == "Все" else _tax_by_dish[_tax_by_dish["TAX_LABEL"] == _tax_filter]
                        _show = _filtered[["DISH_NAME", "TAX_LABEL", "REVENUE", "QTY"]].copy()
                        _show.columns = ["Блюдо", "Ставка", "Выручка ₽", "Кол-во"]
                        _show["Выручка ₽"] = _show["Выручка ₽"].apply(lambda x: f"{x:,.0f}")
        ...
        if not forecast_data.empty and len(forecast_data) >= 2:
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
                    <div>
                        <div style="font-size:.62rem;color:{t3};text-transform:uppercase;letter-spacing:.12em;font-weight:700;">
                            Прогноз на завтра · {dow_names_ru[dow_tomorrow]}, {tomorrow.strftime('%d.%m')}
                        </div>
                        ...
                        <div style="font-size:.72rem;color:{t2};margin-top:2px;">
                            На основе {len(forecast_data)} {dow_names_ru[dow_tomorrow].lower()[:-1]}ов за последние 8 недель
                        </div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:1.2rem;color:{trend_color};font-weight:700;">{trend_arrow} {trend_pct:+.1f}%</div>
                        <div style="font-size:.65rem;color:{t3};">тренд</div>
                    </div>
                </div>
                <div style="display:flex;gap:24px;">
                    <div>
                        <div style="font-size:.6rem;color:{t3};text-transform:uppercase;letter-spacing:.1em;">Заказов</div>
                        <div style="font-size:1.1rem;font-weight:700;color:{t1};">{fc_orders:,}</div>
                    </div>
                    <div>
                        <div style="font-size:.6rem;color:{t3};text-transform:uppercase;letter-spacing:.1em;">Ср. чек</div>
                        <div style="font-size:1.1rem;font-weight:700;color:{t1};">{fc_check:,.0f} ₽</div>
                    </div>
                    <div>
                        <div style="font-size:.6rem;color:{t3};text-transform:uppercase;letter-spacing:.1em;">Гостей</div>
                        <div style="font-size:1.1rem;font-weight:700;color:{t1};">{fc_guests:,}</div>
                    </div>
                    <div>
                        <div style="font-size:.6rem;color:{t3};text-transform:uppercase;letter-spacing:.1em;">Диапазон</div>
            ...
            fig_fc.add_trace(go.Bar(x=fc_chart["DAY"], y=fc_chart["REVENUE"],
                marker_color=accent, opacity=0.7, name="Факт"))
            fig_fc.add_hline(y=fc_rev, line_dash="dash", line_color=accent, opacity=0.5,
                annotation_text=f"Прогноз: {fc_rev:,.0f} ₽", annotation_position="top left")
            fig_fc.update_layout(height=200, title=f"История {dow_names_ru[dow_tomorrow].lower()}ов",
...
"""
            ...
            st.info("Недостаточно данных для прогноза (нужно минимум 2 недели)")
            _forecast_ctx = ""
        ...
        if st.button("Получить рекомендации ИИ", key="ai_strat_btn", use_container_width=True):
...
if page == "Сезонность":
    ...
    if yoy.empty:
        ...
        if prev_year and prev_total > 0:
            st.caption(f"Сравнение общей выручки {cur_year} vs {prev_year}. Разница = {cur_year} минус {prev_year}, % = рост или снижение год к году.")
...
if page == "Рестораны":
    ...
    if not rest_data.empty:
        ...
        st.caption("Точек — сколько столовых передали данные за период. Если точек меньше 31 — часть касс не реплицируется.")
        st.divider()
...
if page == "Персонал":
    ...
    with sub1:
        ...
        if not emp.empty:
            ...
            st.caption("Выручка каждого кассира = сумма оплаченных заказов, которые обслуживал сотрудник.")
    ...
    with sub2:
        ...
        if not clock.empty:
            ...
            st.caption("Опоздание = начало смены позже запланированного времени в графике R-Keeper.")
...
if page == "Касса":
    ...
    if not cash.empty:
        ...
        st.caption("Внесения = наличные в кассу (размен, возвраты). Изъятия = инкассация, закрытие смены.")
...
if page == "Скорость":
    ...
    if not speed.empty:
        ...
        st.caption("Среднее время обслуживания в секундах (от открытия заказа до оплаты). Зелёное число = быстрый, красное = медленный.")
        st.divider()
            ...
            if not card_by_rest.empty:
                ...
                if not problems.empty:
                    ...
                    fig = px.bar(top_rest, x="PROBLEM_COUNT", y="REST_NAME", orientation="h",
                        title="Рестораны с проблемными транзакциями",
        ...
        **Что это:** Статус пробития чеков на ККТ. Показывает фискализированные, нефискализированные, удалённые чеки и ошибки.
        ...
        if not fiscal_sum.empty:
            ...
            if not fiscal_detail.empty:
                ...
                if not problems.empty:
                    st.markdown("#### Рестораны с проблемами")
...
# --- ОТКАЗЫ И ОТМЕНЫ ---
if page == "Удаление":
    page_header("Удаление блюд из чека")
    ...
    with sub_voids:
        ...
        if not voids.empty:
            ...
            st.caption("Удаление блюд — позиции, удалённые из заказа до оплаты. Не влияют на выручку, но важны для контроля.")
            st.divider()
            ...
            if view_mode == "По причинам" and "VOID_REASON" in voids.columns:
                ...
                if "CREATOR_NAME" in voids.columns:
                    ...
                    if not by_cr.empty:
                        st.markdown("#### Кто создаёт удаления")
                        fig = px.bar(by_cr.head(15),x="SUM",y="CREATOR_NAME",orientation="h",title="💸 Сумма удалений по кассирам",color="COUNT",color_continuous_scale="Reds", labels=RU)
                        fig.update_layout(height=500,yaxis=dict(autorange="reversed"),coloraxis_showscale=False,**CHART_THEME)
                        fix_bar_hover(fig)
                ...
                if "AUTHOR_NAME" in voids.columns:
                    ...
                    if not by_au.empty:
                        st.markdown("#### Кто подтверждает удаления (менеджер)")
                ...
                if not by_dish.empty:
                    fig = px.bar(by_dish.head(15),x="SUM",y="DISH_NAME",orientation="h",title="Какие блюда удаляют из чека",color="COUNT",color_continuous_scale="Reds", labels=RU)
                    fig.update_layout(height=500,yaxis=dict(autorange="reversed"),coloraxis_showscale=False,**CHART_THEME)
                    fix_bar_hover(fig)
...
if page == "Склад":
    ...
    if IS_DEMO and _DEMO_DB:
        ...
        with tab_invoices:
            ...
            c1, c2 = st.columns(2)
            with c1: st.metric("Накладных", f"{len(inv)}")
            with c2: st.metric("Сумма", f"{inv['СУММА'].sum():,.0f} ₽" if not inv.empty else "0")
            if not inv.empty:
                fig = px.bar(inv.groupby("ПОСТАВЩИК")["СУММА"].sum().reset_index().sort_values("СУММА", ascending=False),
                    x="СУММА", y="ПОСТАВЩИК", orientation="h", labels={"ПОСТАВЩИК":"Поставщик","СУММА":"₽"})
    ...
    with sub_overview:
        ...
        if tr_err:
            st.caption("⚠️ Перемещения недоступны — нет прав в StoreHouse API")
    ...
    with sub_transfers:
        ...
        if tr_err_r:
            if "MSDSET" in str(tr_err_r) or "errId" in str(tr_err_r) or "Ошибка 5" in str(tr_err_r):
                st.info("Перемещения недоступны — нет прав в StoreHouse API. Обратитесь к администратору Carbis.")
            else:
...
if page == "Накладные":
    ...
    # === DEMO MODE ===
    if IS_DEMO and _DEMO_DB:
        st.caption("Демо-данные: приходные накладные с детализацией")
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1: st.metric("Накладных", f"{len(inv)}")
        with c2: st.metric("Позиций", f"{len(detail):,}")
        with c3: st.metric("Сумма без НДС", f"{inv['PAYSUMNOTAX'].sum():,.0f} ₽" if not inv.empty else "0")
        with c4: st.metric("Поставщиков", f"{inv['SUPPLIER'].nunique()}" if not inv.empty else "0")
        ...
        if not inv.empty:
            ...
            with tab_by_sup:
                ...
                fig = px.bar(by_sup, x="PAYSUMNOTAX", y="SUPPLIER", orientation="h",
                    ...
                    labels={"SUPPLIER":"Поставщик","PAYSUMNOTAX":"Сумма"})
            ...
            with tab_by_prod:
                ...
                if inv_rids:
                    ...
                    fig = px.bar(by_prod.head(20), x="SUM", y="PRODUCT", orientation="h",
                        ...
                        labels={"PRODUCT":"Товар","SUM":"Сумма","QTY":"Кол-во"})
...
if page == "Фудкост (расчёт)":
    ...
    if IS_DEMO and _DEMO_DB:
        ...
        if not selling.empty:
            ...
            c1, c2, c3, c4 = st.columns(4)
            with c1: st.metric("Закупка", f"{total_purchase:,.0f} ₽")
            with c2: st.metric("Продажа", f"{total_selling:,.0f} ₽")
            with c3: st.metric("Фудкост", f"{avg_fc:.1f}%")
            with c4: st.metric("Маржа", f"{total_selling - total_purchase:,.0f} ₽")
            ...
            with tab1:
                ...
                fig = px.bar(top, x="FC", y="DISH_NAME", orientation="h",
                    ...
                    labels={"DISH_NAME": "Товар", "FC": "Фудкост %"})
            ...
            with tab2:
                ...
                fig = px.bar(by_group, x="NAME", y=["PURCHASE", "SELLING"], barmode="group",
                    labels={"NAME": "Группа", "value": "₽", "variable": ""},
                ...
                st.dataframe(by_group[["NAME","PURCHASE","SELLING","FC"]].rename(
                    columns={"NAME":"Группа","PURCHASE":"Закупка","SELLING":"Продажа","FC":"ФК%"}),
                    use_container_width=True, hide_index=True)

            with tab3:
                st.dataframe(selling[["DISH_NAME","QTY","PURCHASE","SELLING","FC","MARGIN"]].rename(
                    columns={"DISH_NAME":"Товар","QTY":"Кол-во","PURCHASE":"Закупка","SELLING":"Продажа","FC":"ФК%","MARGIN":"Маржа"}),
                    use_container_width=True, hide_index=True)
        else:
            st.warning("Нет данных о продажах за период")
...
if page == "Склад: Схема":
    ...
    if IS_DEMO:
        st.info("В демо-режиме API Explorer недоступен. Используйте разделы Склад, Накладные и Фудкост для просмотра складской аналитики.")
        st.stop()
...
if page == "Доход/Расход":
    ...
    st.markdown("### Постоянные расходы (в месяц)")
    ...
    if orders.empty:
        ...
        variable_costs = discounts + staff_meals + purchases
        # НДС (налоги)
        tax_total = 0
        try:
            import re as _re_dr
        ...
        with m1: st.metric("Доход (выручка)", f"{revenue:,.0f} ₽")
        with m2: st.metric("Расходы (всего)", f"{total_costs:,.0f} ₽", delta=f"перем. {variable_costs:,.0f} + НДС {tax_total:,.0f} + пост. {fixed_period:,.0f}")
        with m3: st.metric("Чистый доход", f"{net_profit:,.0f} ₽", delta=f"{margin_pct:.1f}% маржа")
        with m4: st.metric("Ср. в день", f"{daily_net:,.0f} ₽", delta=f"~{monthly_net:,.0f} ₽/мес")
        st.caption("Перем. = скидки + питание сотр. + закупки. НДС рассчитан по ставкам. Пост. = ФОТ + аренда + комм. + маркетинг + прочие (из Личного кабинета). Маржа = чистый доход / выручка.")
        st.divider()
        ...
        with cl:
            ...
            if cost_items:
                ...
                st.info("Заполните постоянные расходы выше")

        with cr:
            st.markdown("### Водопад P&L")
            wf = pd.DataFrame([
                {"Статья": "Выручка", "Сумма": revenue, "m": "absolute"},
                {"Статья": "Закупки", "Сумма": -purchases, "m": "relative"},
                {"Статья": "НДС", "Сумма": -tax_total, "m": "relative"},
        ...
        # === ПО ДНЯМ ===
        st.divider()
        st.markdown("### Выручка по дням vs постоянные расходы")
        ...
        if st.button("Получить рекомендации ИИ", key="ai_fin_btn", use_container_width=True):
